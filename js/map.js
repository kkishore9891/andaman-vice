...
const VMap = (() => {
  ...
  function apply() {
    ...
    // Size everything in SCREEN PIXELS. The SVG uses preserveAspectRatio="meet",
    // so the real on-screen scale is min(cw/vb.w, ch/vb.h) — NOT cw/vb.w. The map
    // is tall+portrait, so on a wide short window the height is what binds; using
    // the width alone made text ~5x too small on desktop.
    const r = svg.getBoundingClientRect();
    const cw = svg.clientWidth || r.width || 1000;
    const ch = svg.clientHeight || r.height || 1000;
    const pxPerUnit = Math.min(cw / vb.w, ch / vb.h) || 1;
